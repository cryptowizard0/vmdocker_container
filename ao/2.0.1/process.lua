...
function Prompt()
  return Colors.green .. Colors.gray
    .. "@" .. Colors.blue .. "aos-" .. process._version .. Colors.gray
    .. "[Inbox:" .. Colors.red .. Colors.gray
    .. "]" .. Colors.reset .. "> "
end
...
In order to print non string types we need to convert to string
  ...
  local data = a
  if ao.outbox.Output and ao.outbox.Output.data then
    -- Convert existing data to string if it's a table
    local existing_data = ao.outbox.Output.data
    if type(existing_data) == "table" then
      existing_data = stringify.format(existing_data)
    end
    data = existing_data .. "\n" .. a
  end
...
local function initializeState(msg, env)
  if not Seeded then
    local module = msg.Module or ""
    local blockHeight = msg['Block-Height'] or "0"
    chance.seed(tonumber(blockHeight .. stringToSeed(msg.Owner .. module .. msg.Id)))
